...
# --- Configuración ---
MODEL_NAME   = os.getenv("SUBLINGO_MODEL", "llama-4-scout-17b-16e-instruct")
...
class LLM:
    ...
    async def complete(self, prompt: str, temperature: float=0.2, max_tokens: int=4096) -> str:
        # Recolecta stream en un hilo para no bloquear asyncio en Windows
        def collect():
            chunks = []
            for ch in self.client.chat.completions.create(
                messages=[{"role":"user","content":prompt}],
                model=self.model, stream=True, temperature=temperature,
...
class TranslatorAgent:
    ...
    Responde con [[id]] <texto>.'''
        ...
        # Regex robusto con raw-string y flags multiline + dotall
...
async def main():
    ...
    sample_text = "\n".join([ev.plaintext for ev in pysubs2.load(files[0], encoding="utf-8") if ev.type=="Dialogue"])[:2000]
    ...
    total_missing = 0
    for f in files:
        await process_subtitle(f, LLM(os.getenv("CEREBRAS_API_KEY"), MODEL_NAME), src_lang, tgt_lang, narrative)
        # process_subtitle ya imprime su propio panel; acumulamos faltantes si se quisiera

    console.print("\n[bold green]Trabajo finalizado[/]")
